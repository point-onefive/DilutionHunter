...
RULES:
1. Use exactly 2 metrics from: $XM insider sales, X insiders selling, stock +X%, CEO/CFO sold, cluster sale, X sales in 30d
2. End with a SHORT meaning clause (2-5 words). VARY these - use DIFFERENT phrases:
   - "dumping into strength"
   - "insiders exiting at highs"
   - "silent exit signal"
...
4. For CEO/CFO sales, emphasize the role
5. For cluster sales (multiple insiders), highlight that
6. Keep total length under 55 characters

Tickers to analyze:
${JSON.stringify(tickerData, null, 2)}

Return ONLY valid JSON object mapping ticker to reason string. Example:
{"TICK1": "$8.2M sold · stock +43% → dumping into strength", "TICK2": "CEO + CFO sales · stock +18% → insiders exiting at highs"}`;
...
function generateFallbackReason(t) {
  ...
  // Second metric
  if (t.hasCEO) {
    parts.push('CEO selling');
  } else if (t.hasCFO) {
    parts.push('CFO selling');
  } else if (t.priceChange30d > 10) {
    parts.push(`stock +${Math.round(t.priceChange30d)}%`);
  } else if (t.isClusterSale) {
    parts.push(`${t.insiderCount} insiders`);
  } else {
    parts.push(`${t.salesCount} sales`);
  }
  
  // Meaning based on metrics
  let meaning;
  if (t.priceChange30d > 30) meaning = 'dumping into strength';
  else if (t.hasCEO || t.hasCFO) meaning = 'confidence questionable';
  else if (t.isClusterSale) meaning = 'cluster exit signal';
  else if (t.priceChange30d > 15) meaning = 'selling into rally';
  else meaning = 'quiet profit-taking';
...
Back next week.`;
  ...
  const lines = leaderboardData.leaderboard.slice(0, 10).map(t => {
    const sold = formatValue(t.metrics.totalValueSold);
    return `#${t.rank} $${t.ticker} — ${sold} sold · Risk: ${t.score}/100\n→ ${t.reason}`;
  });
  
  return `🕵️ WEEKLY INSIDER SELLING WATCH
Insiders selling while price climbs = they know something you don't.
Last 30 days of SEC Form 4 filings.

${lines.join('\n\n')}

Insiders talk bullish. Their trading tells the truth.`;
}
